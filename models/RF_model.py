...
"""Random Forest training script with optional precomputed features and hyperparameter search."""
...
BASE_DIR  = str(PROJECT_ROOT / "runs" / "RF_1016")
# ----------------------------------------------------------------------
...
# Note: see the surrounding code for details.
logger = logging.getLogger("rf_yield")
...
def build_features(df: pd.DataFrame, nBits:int, radius:int, split_name: str = "split", use_desc: bool = False):
    ...
    stats = {
        ...
        'n_bad': n_bad,
        'n_used': X.shape[0],
        'n_features': X.shape[1] if X.shape[0] else base_dim,
    }
    return X, y, stats
...
def hyperparam_search(base_params: Dict[str, Any],
    ...
    rf_est = RandomForestRegressor(random_state=random_state, n_jobs=n_jobs, **base_params)
    ...
    out = pd.DataFrame()
    for key_src, key_dst, neg in [
        ('split0_train_neg_mae', 'train_mae', True),
        ('split0_test_neg_mae',  'val_mae',   True),
        ('split0_train_neg_rmse','train_rmse',True),
        ('split0_test_neg_rmse', 'val_rmse',  True),
        ('split0_train_r2',      'train_r2',  False),
        ('split0_test_r2',       'val_r2',    False),
    ]:
        if key_src in res.columns:
            vals = res[key_src].apply(lambda v: -v if neg else v)
            out[key_dst] = vals

    param_cols = [c for c in res.columns if c.startswith('param_')]
    out = pd.concat([res[param_cols], out], axis=1)
    out['rank_by_val_mae'] = res['rank_test_neg_mae'] if 'rank_test_neg_mae' in res.columns else np.arange(len(out)) + 1

    csv_path = os.path.join(out_dir, 'search_results.csv')
    out.to_csv(csv_path, index=False)
    logger.info(f'Hyperparameter search results saved: {csv_path}')

    best_idx = int(np.argmin(out['val_mae'].values))
...
def main():
    ...
    # Note: see the surrounding code for details.
    parser.add_argument('--train', default=TRAIN_CSV, type=str, help='Training CSV')
    parser.add_argument('--val',   default=VAL_CSV,   type=str, help='Validation CSV')
    parser.add_argument('--test',  default=TEST_CSV,  type=str, help='Test CSV')

    parser.add_argument('--outdir', default=BASE_DIR, type=str, help='Output directory')

    # Note: see the surrounding code for details.
    parser.add_argument('--bits', type=int, default=128, help='Morgan fingerprint length')
    ...
    # Note: see the surrounding code for details.
    parser.add_argument('--n-estimators', type=int, default=600, dest='n_estimators', help='Number of trees')
    parser.add_argument('--max-depth', type=int, default=None, dest='max_depth', help='Maximum depth')
    parser.add_argument('--random-state', type=int, default=42, dest='random_state', help='Random seed')
    parser.add_argument('--n-jobs', type=int, default=-1, dest='n_jobs', help='Number of parallel workers')

    # Note: see the surrounding code for details.
    parser.add_argument('--n-iter-search', type=int, default=25, dest='n_iter_search', help='Random search iterations')
    parser.add_argument('--no-refit', action='store_true', help='Do not refit the final model on train+val')
    ...
    if args.feat_dir:
        ...
        for split in ['train', 'val', 'test']:
            ...
            Yk = f'y_{split}'
            if data[Yk].dtype != np.float32:             # Note: see the surrounding code for details.
        ...
        for name, df in [('train', df_tr), ('val', df_va), ('test', df_te)]:
            ...
            if missing:
                raise ValueError(f'Missing required columns: {name} {missing}')

        X_tr, y_tr, st_tr = build_features(df_tr, nBits=args.bits, radius=args.radius, split_name='train', use_desc=args.use_desc)
        X_va, y_va, st_va = build_features(df_va, nBits=args.bits, radius=args.radius, split_name='val',   use_desc=args.use_desc)
        X_te, y_te, st_te = build_features(df_te, nBits=args.bits, radius=args.radius, split_name='test',  use_desc=args.use_desc)
    ...
    #search_df = pd.read_csv(os.path.join(args.outdir, 'search_results.csv'))
    # Note: see the surrounding code for details.
    #best_row = search_df.iloc[best_idx]
    # Note: see the surrounding code for details.
    best_params = {
    ...
    model_train = RandomForestRegressor(random_state=args.random_state, n_jobs=args.n_jobs, **best_params)
    model_train.fit(X_tr, y_tr)

    val_pred = model_train.predict(X_va)
    mae = mean_absolute_error(y_va, val_pred)
    RMSE = np.sqrt(mean_squared_error(y_va, val_pred))
    R2 = r2_score(y_va, val_pred)
    logger.info(f"VAL(train-fit): MAE/RMSE/R2 = {mae:.3f} / {RMSE:.3f} / {R2:.3f}")

    if not args.no_refit:
        logger.info('Refitting the final model on train+val.')
        final_model = RandomForestRegressor(random_state=args.random_state, n_jobs=args.n_jobs,** best_params)
    ...
    val_pred_final = final_model.predict(X_va)
    mae_f = mean_absolute_error(y_va, val_pred_final)
    RMSE_f = np.sqrt(mean_squared_error(y_va, val_pred_final))
    R2_f = r2_score(y_va, val_pred_final)
    pd.DataFrame({'y_true': y_va, 'y_pred': val_pred_final}).to_csv(VAL_CSV_PATH, index=False)
    logger.info(f"VAL(final): MAE/RMSE/R2 = {mae_f:.3f} / {RMSE_f:.3f} / {R2_f:.3f}")
    ...
    R2_te = r2_score(y_te, te_pred)
    pd.DataFrame({'y_true': y_te, 'y_pred': te_pred}).to_csv(TEST_CSV_PATH, index=False)
    logger.info(f"TEST: MAE/RMSE/R2 = {mae_te:.3f} / {RMSE_te:.3f} / {R2_te:.3f}")
    ...
    meta = {
        ...
        'paths': {
            'train_csv_dir': args.feat_dir if args.feat_dir else None,
    ...
    dump({'model': final_model, 'meta': meta}, MODEL_PATH)
