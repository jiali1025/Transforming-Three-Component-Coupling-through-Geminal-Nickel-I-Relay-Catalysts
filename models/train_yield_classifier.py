...
BASE_DIR = Path(os.getenv("BASE_DIR", str(PROJECT_ROOT / "runs" / "run_cls_v1")))

# -------------- Hyper-parameters -------------
BATCH_SIZE = int(os.getenv("BATCH_SIZE", 256))
NUM_WORKERS = int(os.getenv("NUM_WORKERS", 8))
MAX_STEPS = int(os.getenv("MAX_STEPS", 40_000_000))
...
RUN_NAME = (
    f"NiGAC_cls_bs{BATCH_SIZE}_L{N_ATTN_LAYERS}_{len(GPU_IDS)}g_"
    f"lr{LR}_h{D_HIDDEN}_thr{YIELD_THRESHOLD}"
)
...
logger = logging.getLogger("yield_classifier")
...
logger.info(
    ...
    "layers=%d, threshold=%.2f, max_steps=%d, val_every=%d",
    BATCH_SIZE,
    LR,
    D_HIDDEN,
    N_ATTN_LAYERS,
...
callbacks = [
    ...
    else TQDMProgressBar(refresh_rate=10),
    ckpt_cb,
    EarlyStopping(monitor="val_auroc", patience=PATIENCE, mode="max", verbose=True),
    ThroughputMeter(),
]
...
trainer = L.Trainer(
    ...
    max_steps=MAX_STEPS,
    max_epochs=-1,
    val_check_interval=effective_val_interval,
